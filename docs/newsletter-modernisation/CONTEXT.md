...
- **2026-04-29** — **NEWS-1928 — newsletters list replaces the CPT list at the menu-registration level, not via WP_List_Table override.** The chassis registers `Newsletters_List_Page` as the first submenu under the Newsletters CPT menu and surgically removes the auto-generated `edit.php?post_type=newspack_nl_cpt` "All Newsletters" entry; legacy GETs to that URL are 302'd to the React page. This keeps Add Newsletter, Categories, and Tags submenus intact and lets the React surface own its own URL — no fork of `WP_List_Table`, no parallel rendering paths. The list page registers in **both** modes (bundled and standalone) — only Settings remains mode-gated.
- **2026-04-29** — **Status logic lives server-side via a single REST field, not re-derived in JS.** `Newsletters_List_REST::get_status_for_post` returns `{ kind: 'sent' | 'scheduled' | 'draft' | 'trash', sent_at, scheduled_at }` consolidating `post_status`, `Newspack_Newsletters::is_newsletter_sent()`, the `sending_scheduled` meta flag, and `wp_next_scheduled` semantics. The DataView's Status column reads `kind` directly. The status **filter**, however, maps to native `post_status` (`publish,private` for "Sent", `future` for "Scheduled", etc.) so server-side filtering stays cheap; the rendering/filter-target mismatch is intentional and visually acceptable.
- **2026-04-29** — **No bulk publish / status-change actions on the list.** The base service-provider class hooks `transition_post_status` to dispatch ESP campaigns — bulk publishing or making private would be irreversible. The DataView exposes only `Edit`, `View public page`, `Trash`, `Restore`, and `Delete permanently`. Status changes happen exclusively in the post editor where the existing pre-send confirmation guards apply.
