...
/**
 * Class Newsletters Test Sync_Premium_Subscribers
 *
 ...
 */
class Sync_Premium_Subscribers_Test extends WP_UnitTestCase {
	...
	public function test_cli_sync_premium_subscribers() {
		$expected = [
			[
				self::$users[0]['email'],
		...
		global $cli_sync_premium_subscribers_test_results;
		add_action(
			'newspack_newsletters_update_contact_lists',
			function( $provider, $email, $lists_to_add, $lists_to_remove ) {
				global $cli_sync_premium_subscribers_test_results;
				$cli_sync_premium_subscribers_test_results[] = [ $email, $lists_to_add, $lists_to_remove ];
			},
			10,
			4
		);
		\Newspack_Newsletters\CLI\Sync_Premium_Subscribers::cli_sync_premium_subscribers(
			[],
			[
				'live'    => true,
				'verbose' => true,
		...
		$this->assertEquals( $cli_sync_premium_subscribers_test_results, $expected );
	}
}
