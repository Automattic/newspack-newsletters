...
final class Newspack_Newsletters {
	 ...
	 * blog query that can contain regular posts.
	 *
	...
	public static function should_apply_filter_to_query( $query ) {
		return ! is_admin() && $query->is_main_query() && ( $query->is_archive() || $query->is_search() || $query->is_home() );
	}
	...
	public static function maybe_display_public_archive_posts( $query ) {
		...
		// Allow Newsletter posts to appear alongside regular posts. An empty post_types arg defaults to posts only.
		if ( empty( $post_types ) ) {
			$post_types = [ 'post', self::NEWSPACK_NEWSLETTERS_CPT ];
			$query->set( 'post_type', $post_types ); // phpcs:ignore
		}

		// Filter out non-public Newsletter posts.
		$contains_newsletters   = 'any' === $post_types || self::NEWSPACK_NEWSLETTERS_CPT === $post_types || ( is_array( $post_types ) && in_array( self::NEWSPACK_NEWSLETTERS_CPT, $post_types ) );
		$contains_regular_posts = empty( $post_types ) || 'any' === $post_types || 'post' === $post_types || ( is_array( $post_types ) && in_array( 'post', $post_types ) );

		if ( $contains_newsletters ) {
			...
			// If the query contains regular posts, also allow posts that don't have the is_public meta field.
