		...
		return (
			<Fragment>
				...
				{ showTestModal && (
					...
					>
						<TextControl
							label={ __( 'Send to these emails', 'newspack-newsletters' ) }
							value={ testEmails }
							onChange={ value => this.setState( { testEmails: value } ) }
						/>
						<Button
							isPrimary
							onClick={ () =>
				...
				<SelectControl
					...
					value={ list_id }
					options={ [
						{ value: null, label: __( '-- Select A List --', 'newspack-newsletters' ) },
						...lists.map( ( { id, name } ) => ( { value: id, label: name } ) ),
					] }
				...
				<SelectControl
					...
					value={ template_id }
					options={ [
						{ value: null, label: __( '-- Select A Template --', 'newspack-newsletters' ) },
						...templates.map( ( { id, name } ) => ( { value: id, label: name } ) ),
					] }
