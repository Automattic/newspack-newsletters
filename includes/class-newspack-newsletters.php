...
final class Newspack_Newsletters {
	...
	public function enqueue_quick_edit_population( $pagehook ) {
		...
		wp_enqueue_script( 'populatequickedit', plugins_url( '../dist/quickedit.js', __FILE__ ), array( 'jquery' ), '1.0', true );
	 ...
	 * Adding column
	 * 
	 * @param array $column_array apassing columns.
	 * 
	 * @return mixed
	 */
	...
	public function quick_edit_save( $post_id ) {
		...
		// check nonce.
		if ( isset( $_POST['npublic_nonce'] ) ) {
			$pnonce = sanitize_key( $_POST['npublic_nonce'] );
		}
		if ( ! empty( $pnonce ) ) {
			if ( wp_verify_nonce( $pnonce, 'quick_edit_npublic_nonce' ) ) {
	...
	public static function bulk_action_handler( $redirect_to, $doaction, $post_ids ) {
		...
		if ( 'public_newsletters' == $doaction ) {
		...
		if ( 'non_public_newsletters' == $doaction ) {
			foreach ( $post_ids as $post_id ) {
				update_post_meta( $post_id, 'is_public', '' );
			}
		...
		if ( ! empty( $nonce ) ) {
			if ( wp_verify_nonce( $nonce, 'npublic' ) ) {
				...
				if ( ! empty( $_REQUEST['public_newsletters_done'] ) ) {
					...
					printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( __( 'Published %d newsletters to public.', 'newspack-newsletters' ) ) . '</p></div>', esc_html( $num_changed ) );
				} elseif ( ! empty( $_REQUEST['non-public_newsletters_done'] ) ) {
					$num_changed = (int) $_REQUEST['non-public_newsletters_done'];
					/* translators: %d is replaced with "integer" */
					printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( __( '%d newsletters to non public.', 'newspack-newsletters' ) ) . '</p></div>', esc_html( $num_changed ) );
				}
			}
		}
	}
