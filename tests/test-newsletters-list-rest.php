...
class Newsletters_List_REST_Test extends WP_UnitTestCase {
	...
	/**
	 * Quick Edit authors endpoint denies anonymous users.
	 */
	public function test_rest_quick_edit_authors_permission_check_denies_anonymous() {
		wp_set_current_user( 0 );
		$this->assertFalse( Newsletters_List_REST::rest_quick_edit_authors_permission_check() );
	}

	/**
	 * Quick Edit authors endpoint denies users who can edit their own
	 * newsletters but lack `edit_others_posts` — the gate is "can you
	 * reassign authorship?", not just "can you edit newsletters?".
	 * Stops a user-enumeration leak that the broader `edit_posts` gate
	 * would have allowed.
	 */
	public function test_rest_quick_edit_authors_permission_check_denies_author_without_edit_others_posts() {
		$author_id = self::factory()->user->create( [ 'role' => 'author' ] );
		wp_set_current_user( $author_id );
		$this->assertTrue( current_user_can( 'edit_posts' ), 'precondition: author has edit_posts' );
		$this->assertFalse( current_user_can( 'edit_others_posts' ), 'precondition: author lacks edit_others_posts' );
		$this->assertFalse( Newsletters_List_REST::rest_quick_edit_authors_permission_check() );
	}

	/**
	 * Quick Edit authors endpoint allows users with `edit_others_posts`
	 * — the cap that lets them actually reassign authorship.
	 */
	public function test_rest_quick_edit_authors_permission_check_allows_editor() {
		$editor_id = self::factory()->user->create( [ 'role' => 'editor' ] );
		wp_set_current_user( $editor_id );
		$this->assertTrue( Newsletters_List_REST::rest_quick_edit_authors_permission_check() );
	}

	/**
	 * Authorized users receive a list of `{id, name}` pairs ordered by
	 * display_name ASC, covering every user with `edit_posts` on the
	 * newsletter CPT.
	 */
	public function test_rest_get_quick_edit_authors_returns_id_name_payload_ordered_by_display_name() {
		// Three users: an editor (so the caller is authorised), plus an
		// author + another editor so we can assert the alphabetical sort.
		$alpha = self::factory()->user->create(
			[
				'role'         => 'editor',
				'display_name' => 'Alpha Author',
			]
		);
		$bravo = self::factory()->user->create(
			[
				'role'         => 'author',
				'display_name' => 'Bravo Author',
			]
		);
		$charlie = self::factory()->user->create(
			[
				'role'         => 'editor',
				'display_name' => 'Charlie Author',
			]
		);

		wp_set_current_user( $alpha );
		$response = Newsletters_List_REST::rest_get_quick_edit_authors();
		...
		$this->assertContains( $bravo, $ids );
		$this->assertContains( $charlie, $ids );

		// The three test users should appear in display-name order. Other
		// users created by the test runner (admin) may interleave; what we
		// assert is the relative order between our three.
		$alpha_pos   = array_search( $alpha, $ids, true );
		$bravo_pos   = array_search( $bravo, $ids, true );
		$charlie_pos = array_search( $charlie, $ids, true );
		$this->assertLessThan( $bravo_pos, $alpha_pos, 'Alpha should sort before Bravo' );
		$this->assertLessThan( $charlie_pos, $bravo_pos, 'Bravo should sort before Charlie' );
	}
}
